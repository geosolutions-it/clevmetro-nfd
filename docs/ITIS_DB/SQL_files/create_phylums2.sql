...
SELECT t.* FROM crosstab(
   'SELECT tsn, complete_name, rank_id, parent_tsn, kingdom_id, rn, vernacular_name
     FROM  (
        ...
        FROM (SELECT phylum_tsn FROM cmiphylums GROUP BY phylum_tsn) f JOIN taxonomic_units u ON f.phylum_tsn = u.tsn JOIN vernaculars v ON u.tsn = v.tsn
